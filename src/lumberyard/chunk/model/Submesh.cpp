...
namespace dragon::lumberyard::chunk::model {
    Submesh::Submesh(Array<char>* buffer, CRCH_CHUNK_HEADER chunk_header) {
        ...
        if (((uint32_t)Header.Flags & (uint32_t)SUBMESH_HEADER::FLAGS::BoneIndices) == (uint32_t)SUBMESH_HEADER::FLAGS::BoneIndices) {
            Bones = buffer->lpcast<SUBMESH_BONE>(&ptr, Header.Count);
        }
        if (((uint32_t)Header.Flags & (uint32_t)SUBMESH_HEADER::FLAGS::SubsetTexelDensity) == (uint32_t)SUBMESH_HEADER::FLAGS::SubsetTexelDensity) {
