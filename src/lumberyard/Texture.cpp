...
namespace dragon::lumberyard {
    ...
    bool
    Texture::noesis_load([[maybe_unused]] BYTE* buffer, [[maybe_unused]] int length, CArrayList<noesisTex_t*> &noe_tex,
                         noeRAPI_t* rapi) {
        ...
        if(data.empty())
            return false;
        noesisTex_t* tex = rapi->Noesis_LoadTexByHandler(reinterpret_cast<BYTE*>(data.data()), data.size(),
                                                         (char*) ".dds");
        if(tex == nullptr)
            return false;
        noe_tex.Append(tex);
